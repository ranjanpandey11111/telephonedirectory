...
console.log(req.body)
  let newUser = new User({
    ...
    telephone: sanitizeTelephone(req.body.telephone),
    ...
    .catch((err) => {
      if (err.errors) {
        ...
        if (err.errors.telephone) {
          res.status(400).json({ success: false, msg: err.errors.telephone.message });
          return;
        }
        if (err.errors.age) {
  ...
  User.findOneAndUpdate({ _id: req.params.id }, updatedUser, { runValidators: true, context: 'query' })
    ...
    .catch((err) => {
      if (err.errors) {
        ...
        if (err.errors.telephone) {
          res.status(400).json({ success: false, msg: err.errors.telephone.telephone });
          return;
        }
        if (err.errors.age) {
          res.status(400).json({ success: false, msg: err.errors.age.message });
...
sanitizeTelephone = (telephone) => {
  // Return empty if age is non-numeric
  if (telephone === '') return '';
  return (telephone === '') ? telephone : parseInt(telephone);
}
sanitizeAge = (age) => {
  // Return empty if age is non-numeric
